...
import { ProcessState, ExecutionResult, StateAction } from '../state/WorkflowState.js';
...
export class WorkflowEngine {
   ...
   * 启动一个新的流程实例
   */
  startProcess(definition: ProcessDefinition, initialData: Record<string, any> = {}): ProcessState {
    // 创建初始状态
    ...
    // 应用启动动作得到新状态
    const updatedState = this.transitionEngine.applyAction(initialState, startAction);
   ...
   * 执行流程动作（如完成任务、触发事件等）
   */
  executeAction(currentState: ProcessState, action: StateAction): ExecutionResult {
    try {
      // 使用转换引擎应用动作
      const newState = this.transitionEngine.applyAction(currentState, action);
      ...
      const events = this.generateEvents(currentState, executedState, action);
      ...
      const errorState = this.transitionEngine.applyAction(currentState, errorAction);
   ...
   * 创建初始状态
   */
  private createInitialState(definition: ProcessDefinition, initialData: Record<string, any>): ProcessState {
    return {
      id: this.generateId(),
   ...
   * 生成事件
   */
  private generateEvents(oldState: ProcessState, newState: ProcessState, action: StateAction): any[] {
    const events = [];
