...
export class TransitionEngine {
  ...
  applyAction(state: ProcessState, action: StateAction): ProcessState {
    ...
    switch (action.type) {
      ...
      case 'EXECUTE_ELEMENT':
        return this.handleExecuteElement(state, action, newHistoryEntry);
      
      case 'COMPLETE_TASK':
        return this.handleCompleteTask(state, action, newHistoryEntry);
      
      case 'TRANSITION_TOKEN':
        return this.handleTransitionToken(state, action, newHistoryEntry);
   ...
   * 验证状态转换是否有效
   */
  validateTransition(currentState: ProcessState, action: StateAction): boolean {
    // 检查基本约束
    ...
    switch (action.type) {
      ...
      case 'COMPLETE_TASK':
        ...
        if (taskId) {
          ...
          return !!task && (task.status === 'wait' || task.status === 'active');
        }
      ...
      case 'TRANSITION_TOKEN':
        ...
        if (tokenId) {
          return currentState.tokens.some(token => token.id === tokenId);
        }
        return true;
   ...
   * 执行状态转换
   */
  applyTransition(currentState: ProcessState, action: StateAction): ProcessState {
    // 验证转换
    if (!this.validateTransition(currentState, action)) {
      throw new Error(`Invalid transition: ${action.type} from state ${currentState.status}`);
    }
   ...
   * 处理开始流程动作
   */
  private handleStartProcess(state: ProcessState, action: StateAction, historyEntry: any): ProcessState {
    return {
      ...state,
      status: 'running',
   ...
   * 处理执行元素动作
   */
  private handleExecuteElement(state: ProcessState, action: StateAction, historyEntry: any): ProcessState {
    return {
      ...state,
      data: { ...state.data, ...action.payload.newData },
      tokens: state.tokens.filter(t => t.id !== action.payload.tokenId),
      items: action.payload.item ? [...state.items, action.payload.item] : state.items,
      history: [...state.history, historyEntry]
    };
  }
   ...
   * 处理完成任务动作
   */
  private handleCompleteTask(state: ProcessState, action: StateAction, historyEntry: any): ProcessState {
    return {
      ...state,
   ...
   * 处理令牌流转动作
   */
  private handleTransitionToken(state: ProcessState, action: StateAction, historyEntry: any): ProcessState {
    return {
      ...state,
      tokens: [
   ...
   * 处理数据更新动作
   */
  private handleUpdateData(state: ProcessState, action: StateAction, historyEntry: any): ProcessState {
    return {
      ...state,
   ...
   * 处理错误发生动作
   */
  private handleErrorOccurred(state: ProcessState, action: StateAction, historyEntry: any): ProcessState {
    return {
      ...state,
      history: [
        ...state.history, 
  ...
  private getActionTypeForHistory(actionType: string): string {
    switch (actionType) {
      case 'START_PROCESS': return 'start';
      case 'EXECUTE_ELEMENT': return 'transition';
      case 'COMPLETE_TASK': return 'complete';
      case 'TRANSITION_TOKEN': return 'transition';
