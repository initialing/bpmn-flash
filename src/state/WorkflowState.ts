...
export interface StateAction {
  type: 'START_PROCESS' | 'EXECUTE_ELEMENT' | 'COMPLETE_TASK' | 'TRANSITION_TOKEN' | 'UPDATE_DATA' | 'ERROR_OCCURRED';
  payload: any;
  timestamp: Date;
}
...
export interface ProcessEvent {
  type: 'TASK_CREATED' | 'TASK_COMPLETED' | 'PROCESS_STARTED' | 'PROCESS_COMPLETED' | 'TOKEN_MOVED' | 'GATEWAY_EVALUATED';
  payload: any;
  timestamp: Date;
}
...
export class WorkflowState {
   ...
   * 创建初始流程状态
   */
  static createInitialState(definition: ProcessDefinition, initialData: Record<string, any> = {}): ProcessState {
    return {
      id: this.generateId(),
   ...
   * 将动作类型转换为历史记录类型
   */
  private static getActionTypeForHistory(actionType: string): ExecutionHistoryEntry['action'] {
    switch (actionType) {
