...
export class TaskGenerator {
   ...
   * 为令牌创建对应的任务
   */
  private static createTaskForToken(state: ProcessState, token: any): Item | null {
    // 这里需要访问流程定义来确定元素类型
    // 暂时返回null，等待与解析器集成
    return null;
  }
   ...
   * 生成任务完成后的后续任务
   */
  static generateFollowUpTasks(state: ProcessState, completedTaskId: string): Item[] {
...
export class TaskComputer {
  ...
  static computeCurrentTasks(state: ProcessState): Item[] {
    ...
    const existingActiveTasks = state.items.filter(item => 
  ...
  static computeCompletionPercentage(state: ProcessState): number {
    ...
    const completedItems = state.items.filter(item => item.status === 'completed').length;
