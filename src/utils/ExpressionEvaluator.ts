...
export function evaluateExpression(expression: string | null, context: Record<string, any>): boolean {
  if (!expression) {
  ...
  try {
    ...
    if (expression.startsWith('${') && expression.endsWith('}')) {
      const innerExpression = expression.substring(2, expression.length - 1).trim();
...
function evaluateSimpleExpression(expr: string, context: Record<string, any>): boolean {
  ...
  if (expr.includes('&&')) {
    ...
    return parts.every(part => evaluateSimpleExpression(part.trim(), context));
  }

  if (expr.includes('||')) {
    const parts = splitByOperator(expr, '||');
    return parts.some(part => evaluateSimpleExpression(part.trim(), context));
  }
  ...
  if (expr.includes('==')) {
    ...
    return getValue(left.trim(), context) == getValue(right.trim(), context);
  }
  ...
  if (expr.includes('>=')) {
    const [left, right] = splitComparison(expr, '>=');
    return Number(getValue(left.trim(), context)) >= Number(getValue(right.trim(), context));
  }
...
function splitByOperator(expr: string, operator: string): string[] {
  ...
  while (i < expr.length) {
    ...
    if (char === '(') {
      ...
      parenLevel--;
      current += char;
    } else if (parenLevel === 0 && expr.substr(i, operator.length) === operator) {
      result.push(current);
      current = '';
...
function splitComparison(expr: string, operator: string): [string, string] {
  ...
  if (index > 0) {
    return [expr.substring(0, index), expr.substring(index + operator.length)];
  }
